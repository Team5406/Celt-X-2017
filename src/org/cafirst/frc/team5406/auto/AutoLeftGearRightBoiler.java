...
public class AutoLeftGearRightBoiler  extends AutonomousRoutine{
	...
	public void periodic(){
		...
		case 0:
			...
			if( direction*robotPosition[0] > ((151-Constants.ROBOT_LENGTH)/(Constants.WHEEL_DIAM*Math.PI))){
				autoStep = 1;
				robotIntake.dropGear(false);
				robotDrive.resetPosition();
				robotDrive.driveAtAngleUpdate(0.0, 60.0, true);
			} else if( direction*robotPosition[0] > ((125-Constants.ROBOT_LENGTH)/(Constants.WHEEL_DIAM*Math.PI))){
				robotDrive.driveAtAngleUpdate(115, 60, true);
			}else if ( direction*robotPosition[0] > ((85-Constants.ROBOT_LENGTH)/(Constants.WHEEL_DIAM*Math.PI))){
