...
public class AutoLeftGearRightBoiler extends AutonomousRoutine{
	...
	/**
	 * Constructor for Left Gear Right Boiler Auto
	 * @param robotDrive The Robot Drive System
	 * @param robotIntake The Robot Intake System
	 */
	public AutoLeftGearRightBoiler(Drive robotDrive, Intake robotIntake)
	{
	...
	public void periodic()
	{
		...
		{
		case 0:
			if(Math.abs(direction * robotDistances[1]) > ((35) / (Constants.WHEEL_DIAM * Math.PI)))
			{
		...
		case 2:
			if(Math.abs(direction * robotDistances[1]) > ((28) / (Constants.WHEEL_DIAM * Math.PI)))
			{
			...
			else if(Math.abs(direction * robotDistances[1]) > ((12) / (Constants.WHEEL_DIAM * Math.PI)))
	...
	public AutonomousRoutine newInstance() {
		return new AutoLeftGearRightBoiler(robotDrive, robotIntake);
	}
}
