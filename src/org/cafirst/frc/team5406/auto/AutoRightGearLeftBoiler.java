...
public class AutoRightGearLeftBoiler extends AutonomousRoutine{
	...
	/**
	 * Constructor for Right Gear Left Boiler Auto
	 * @param robotDrive The Robot Drive System
	 * @param robotIntake The Robot Intake System
	 */
	public AutoRightGearLeftBoiler(Drive robotDrive, Intake robotIntake)
	{
	...
	public void periodic()
	{
		...
		{
		case 0:
			if(Math.abs(direction * robotDistances[1]) > ((45) / (Constants.WHEEL_DIAM * Math.PI)))
			{
				robotDrive.driveAtCurveStart(300, 48, -55, 8);
		...
		case 2:
			if(Math.abs(direction * robotDistances[1]) > ((37) / (Constants.WHEEL_DIAM * Math.PI)))
			{
			...
			else if(Math.abs(direction * robotDistances[1]) > ((18) / (Constants.WHEEL_DIAM * Math.PI)))
	...
	public AutonomousRoutine newInstance() {
		return new AutoRightGearLeftBoiler(robotDrive, robotIntake);
	}
}
