...
public class MotionProfile {
	...
	public MotionProfile(CANTalon talon, double[][] profile, int totalCnt) {
		...
		inst = new instrumentation(_talon.getDeviceID());
	}
	...
	public void control() {
		...
		//System.out.println(_talon.getDeviceID());
	...
	private void startFilling() {
		...
		for (int i = 0; i < _totalCnt; ++i) {
			...
			System.out.println(_profile[i][0]);
			/*if(_profile[i][1] < 60 && _profile[i][1]!=0){
				point.velocity = 60;
			}else{*/
				point.velocity = _profile[i][1];
			//}
			point.timeDurMs = (int) _profile[i][2];
